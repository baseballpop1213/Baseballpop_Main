...
/* -------------------------------------------------------------------------- */
/*                           COLLEGE ATHLETIC SKILLS                          */
/* -------------------------------------------------------------------------- */
/**
 * College ATHLETIC
 *
 * Same as HS except:
 * - ASPJUMP max: 33 inches
 * - ASPSCP max: 33 ft
 * - ASPSUP max: 33 ft
 *
 * 1B / 4B speed, strength, balance, mobility = HS.
 */

const TS1B_MAX_POINTS_COLLEGE = 60;
const TS4B_MAX_POINTS_COLLEGE = 65;
const SPEED_POINTS_MAX_COLLEGE =
  TS1B_MAX_POINTS_COLLEGE + TS4B_MAX_POINTS_COLLEGE; // 125

const PUSHUPS_MAX_POINTS_COLLEGE = 25; // APUSH60 / 3, max 75 reps
const SITUPS_MAX_POINTS_COLLEGE = 25; // ASIT60 / 4, max 100 reps
const PULLUPS_MAX_POINTS_COLLEGE = 25; // APULL60, 1 pt per pull-up up to 25

const VJUMP_MAX_POINTS_COLLEGE = 33; // ASPJUMP, inches

const ASPSCP_MAX_POINTS_COLLEGE = 33; // seated chest pass, ft
const ASPSUP_MAX_POINTS_COLLEGE = 33; // sit-up and throw, ft

const SLS_OPEN_MAX_COLLEGE = 10;
const SLS_CLOSED_MAX_COLLEGE = 15;

const MSR_TOTAL_MAX_COLLEGE = 6;
...
const ATHLETIC_POINTS_MAX_COLLEGE =
  ...
  SLS_OPEN_MAX_COLLEGE +
  SLS_CLOSED_MAX_COLLEGE +
  MSR_TOTAL_MAX_COLLEGE +
  TOE_TOUCH_MAX_COLLEGE +
  DEEP_SQUAT_MAX_COLLEGE; // 345

function computeCollegeAthleticSkills(metrics: MetricMap) {
  ...
  const run1bPoints = clamp(
    run1bPointsRaw,
    0,
    TS1B_MAX_POINTS_COLLEGE
  );
  const run4bPoints = clamp(
    run4bPointsRaw,
    0,
    TS4B_MAX_POINTS_COLLEGE
  );

  const speedPointsTotal = sum([run1bPoints, run4bPoints]);
  const speedScore =
    speedPointsTotal !== null
      ? Number(((speedPointsTotal / SPEED_POINTS_MAX_COLLEGE) * 50).toFixed(1))
      : null;
  ...
  const slsOpenPoints = clamp(
    slsOpenPointsRaw,
    0,
    SLS_OPEN_MAX_COLLEGE
  );
  const slsClosedPoints = clamp(
    slsClosedPointsRaw,
    0,
    SLS_CLOSED_MAX_COLLEGE
  );
  ...
  const pushupsPoints = clamp(
    pushupsPointsRaw,
    0,
    PUSHUPS_MAX_POINTS_COLLEGE
  );
  const situpsPoints = clamp(
    situpsPointsRaw,
    0,
    SITUPS_MAX_POINTS_COLLEGE
  );
  const pullupsPoints = clamp(
    pullupsPointsRaw,
    0,
    PULLUPS_MAX_POINTS_COLLEGE
  );
  ...
  const msrPointsClamped =
    msrPointsTotal !== null
      ? Math.max(0, Math.min(MSR_TOTAL_MAX_COLLEGE, msrPointsTotal))
      : null;
  ...
  const toeTouchPoints = clamp(
    toeTouchRawPoints,
    0,
    TOE_TOUCH_MAX_COLLEGE
  );
  const deepSquatPoints = clamp(
    deepSquatRawPoints,
    0,
  ...
  const athleticScore =
    athleticTotalPoints !== null
      ? Number(
          ((athleticTotalPoints / ATHLETIC_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;
...
/* -------------------------------------------------------------------------- */
/**
 * All hitting tests & maxima same as HS.
 */

const TEE_LD_MAX_POINTS_COLLEGE = 10;        // HC10LD
const PITCH_QUALITY_MAX_POINTS_COLLEGE = 50; // H10FAST
const EXIT_VELO_MAX_POINTS_COLLEGE = 24;     // HPTEV
const BAT_SPEED_MAX_POINTS_COLLEGE = 25;     // HBSPEED
const VARIED_SPEED_MAX_POINTS_COLLEGE = 25;  // H5VSPD
const CURVEBALL_MAX_POINTS_COLLEGE = 25;     // H5CBPD

const CONTACT_POINTS_MAX_COLLEGE =
  TEE_LD_MAX_POINTS_COLLEGE +
  PITCH_QUALITY_MAX_POINTS_COLLEGE +
  VARIED_SPEED_MAX_POINTS_COLLEGE +
  CURVEBALL_MAX_POINTS_COLLEGE; // 110

const POWER_POINTS_MAX_COLLEGE =
  EXIT_VELO_MAX_POINTS_COLLEGE +
  PITCH_QUALITY_MAX_POINTS_COLLEGE +
  BAT_SPEED_MAX_POINTS_COLLEGE; // 99

const HITTING_POINTS_MAX_COLLEGE =
  TEE_LD_MAX_POINTS_COLLEGE +
  PITCH_QUALITY_MAX_POINTS_COLLEGE +
  EXIT_VELO_MAX_POINTS_COLLEGE +
  BAT_SPEED_MAX_POINTS_COLLEGE +
  VARIED_SPEED_MAX_POINTS_COLLEGE +
  CURVEBALL_MAX_POINTS_COLLEGE; // 159

function computeCollegeHitting(metrics: MetricMap) {
  ...
  const pitchPoints = clamp(
    fastballRaw,
    0,
    PITCH_QUALITY_MAX_POINTS_COLLEGE
  );
  const teeLdPoints = clamp(
    teeLdRaw,
    0,
    TEE_LD_MAX_POINTS_COLLEGE
  );
  ...
  const curveballPoints = clamp(
    curveballRaw,
    0,
    CURVEBALL_MAX_POINTS_COLLEGE
  );
  ...
  const contactScore =
    contactRawPoints !== null
      ? Number(
          ((contactRawPoints / CONTACT_POINTS_MAX_COLLEGE) * 100).toFixed(1)
        )
      : null;
  ...
  const powerScore =
    powerRawPoints !== null
      ? Number(
          ((powerRawPoints / POWER_POINTS_MAX_COLLEGE) * 100).toFixed(1)
        )
      : null;
  ...
  const hittingScore =
    hittingTotalPoints !== null
      ? Number(
          ((hittingTotalPoints / HITTING_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;

  return {
    score: hittingScore,
    max_points: HITTING_POINTS_MAX_COLLEGE,
 ...
 *
 * Same as HS but:
 * - TSPEED max points: 50 (100 mph / 2)
 * - Logistic params for STRIKEOUTCHANCE:
 *   baseline = -3.75
 *   a_s = 1.4
 *   a_a = 1.2
 *   a_p = 0.32
 ...
 *   maxSpeed = 100 mph
 *
 * Additional pitches (TPITCH5AP1, 2, 3, ...) are supported the same way as HS:
 * we scan keys starting with "tpitch5ap".
 */

const PITCH_TSPEED_POINTS_MAX_COLLEGE = 50;  // 100 / 2
const PITCH_ACC_POINTS_MAX_COLLEGE = 60;     // TPITCH2060
const PITCH_POINTS_MAX_COLLEGE =
  PITCH_TSPEED_POINTS_MAX_COLLEGE + PITCH_ACC_POINTS_MAX_COLLEGE; // 110

const ADDITIONAL_PITCH_POINTS_MAX_COLLEGE = 15;

const STRIKE_BASELINE_COLLEGE = -3.75;
const STRIKE_WEIGHT_SPEED_COLLEGE = 1.4;
const STRIKE_WEIGHT_ACC_COLLEGE = 1.2;
const STRIKE_WEIGHT_P_COLLEGE = 0.32;
const STRIKE_WEIGHT_AP_ACC_COLLEGE = 0.75;
const STRIKE_MIN_SPEED_COLLEGE = 70;
const STRIKE_MAX_SPEED_COLLEGE = 100;

function computeCollegePitching(metrics: MetricMap) {
  ...
  const tspeedPoints = clamp(
    tspeedPointsRaw,
    0,
    PITCH_TSPEED_POINTS_MAX_COLLEGE
  );

  const tpitchPoints = clamp(
    tpitchRaw,
    0,
    PITCH_ACC_POINTS_MAX_COLLEGE
  );
  ...
  for (const key of Object.keys(metrics as any)) {
    ...
    const clamped = clamp(
      raw,
      0,
      ADDITIONAL_PITCH_POINTS_MAX_COLLEGE
    );
  ...
  const additionalPitchAccuracyNormalized =
    additionalPitchAveragePoints !== null
      ? Math.max(
          ...
          Math.min(
            1,
            additionalPitchAveragePoints / ADDITIONAL_PITCH_POINTS_MAX_COLLEGE
          )
        )
      : 0;
  ...
  const pitchingScore =
    pitchingTotalPoints !== null
      ? Number(
          ((pitchingTotalPoints / PITCH_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;
  ...
  const pitchSpeedScorePercent =
    tspeedPoints !== null
      ? Number(
          (
            (tspeedPoints / PITCH_TSPEED_POINTS_MAX_COLLEGE) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  const pitchAccScorePercent =
    tpitchPoints !== null
      ? Number(
          (
            (tpitchPoints / PITCH_ACC_POINTS_MAX_COLLEGE) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  const pitchScorePercent =
    tspeedPoints !== null && tpitchPoints !== null
      ? Number(
          (
            ((tspeedPoints + tpitchPoints) / PITCH_POINTS_MAX_COLLEGE) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  if (tspeedMph !== null && tpitchPoints !== null) {
    const nSRaw =
      (tspeedMph - STRIKE_MIN_SPEED_COLLEGE) /
      (STRIKE_MAX_SPEED_COLLEGE - STRIKE_MIN_SPEED_COLLEGE);
    ...
    const accNorm =
      tpitchPoints !== null
        ? Math.max(
            0,
            Math.min(1, tpitchPoints / PITCH_ACC_POINTS_MAX_COLLEGE)
          )
        : 0;
    ...
    const x =
      STRIKE_BASELINE_COLLEGE +
      STRIKE_WEIGHT_SPEED_COLLEGE * nS +
      STRIKE_WEIGHT_ACC_COLLEGE * accNorm +
      STRIKE_WEIGHT_P_COLLEGE * P +
      STRIKE_WEIGHT_AP_ACC_COLLEGE * AA;
  ...
  return {
    score: pitchingScore,
    max_points: PITCH_POINTS_MAX_COLLEGE,
...
/* -------------------------------------------------------------------------- */
/*                       COLLEGE CATCHER / 1B / IF / OF                       */
/* -------------------------------------------------------------------------- */
/**
 * These are identical to HS in both maxima and calculations.
 */

/* CATCHER */
const C20PCS_POINTS_MAX_COLLEGE = 40;
const CT2BT_POINTS_MAX_COLLEGE = 15;
const CTTT2B_POINTS_MAX_COLLEGE = 15;
const CATCHER_POINTS_MAX_COLLEGE =
  C20PCS_POINTS_MAX_COLLEGE +
  CT2BT_POINTS_MAX_COLLEGE +
  CTTT2B_POINTS_MAX_COLLEGE; // 70

function computeCollegeCatcher(metrics: MetricMap) {
  ...
  if (ct2btSeconds !== null) {
    ...
    if (ct2btPoints > CT2BT_POINTS_MAX_COLLEGE) {
      ct2btPoints = CT2BT_POINTS_MAX_COLLEGE;
    }
  }

  const cttt2bPoints = clamp(
    cttt2bRaw,
    0,
    CTTT2B_POINTS_MAX_COLLEGE
  );

  const catcherTotalPoints = sum([
    c20pcsPoints,
    ct2btPoints,
    cttt2bPoints,
  ]);
  ...
  const catcherScore =
    catcherTotalPoints !== null
      ? Number(
          ((catcherTotalPoints / CATCHER_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;

  return {
    score: catcherScore,
    max_points: CATCHER_POINTS_MAX_COLLEGE,
...
/* FIRST BASE */
const C101B_POINTS_MAX_COLLEGE = 30;
const C1BST_POINTS_MAX_COLLEGE = 15;
const RLC_POINTS_MAX_COLLEGE = 12;
const FBFLY_POINTS_MAX_COLLEGE = 6;
const FBLD_POINTS_MAX_COLLEGE = 6;
const FIRSTBASE_POINTS_MAX_COLLEGE =
  C101B_POINTS_MAX_COLLEGE +
  C1BST_POINTS_MAX_COLLEGE +
  RLC_POINTS_MAX_COLLEGE +
  FBFLY_POINTS_MAX_COLLEGE +
  FBLD_POINTS_MAX_COLLEGE; // 69

function computeRlcTotalForPrefixCollege(metrics: MetricMap, prefix: string) {
  ...
  const total =
    totalRaw !== null
      ? Math.max(0, Math.min(RLC_POINTS_MAX_COLLEGE, totalRaw))
      : null;

  return { pointsByRep: points, total };
}

function computeCollegeFirstBase(metrics: MetricMap) {
  const c101bRaw = getMetric(metrics, "c101b_catching_test");
  const c1bstRaw = getMetric(metrics, "c1bst_scoops_test");

  const c101bPoints = clamp(
    c101bRaw,
    0,
    C101B_POINTS_MAX_COLLEGE
  );
  const c1bstPoints = clamp(
    c1bstRaw,
    0,
    C1BST_POINTS_MAX_COLLEGE
  );

  const rlc1b = computeRlcTotalForPrefixCollege(metrics, "rlc1b");

  const fbFlyRaw = getMetric(metrics, "fbfly_points");
  const fbLdRaw = getMetric(metrics, "fbld_points");

  const fbFlyPoints = clamp(
    fbFlyRaw,
    0,
    FBFLY_POINTS_MAX_COLLEGE
  );
  const fbLdPoints = clamp(
    fbLdRaw,
    0,
    FBLD_POINTS_MAX_COLLEGE
  );
  ...
  const firstBaseScore =
    firstBaseTotalPoints !== null
      ? Number(
          ((firstBaseTotalPoints / FIRSTBASE_POINTS_MAX_COLLEGE) * 50).toFixed(
            1
          )
        )
      : null;
  ...
  return {
    ...
    max_points: FIRSTBASE_POINTS_MAX_COLLEGE,
...
/* INFIELD */
const IF_FLY_POINTS_MAX_COLLEGE = 6;
const IFSS1BT_POINTS_MAX_COLLEGE = 14.5;
const INFIELD_POINTS_MAX_COLLEGE = 86.5;

function timeTo14_5PointsCollege(seconds: number | null): number | null {
  if (seconds === null) return null;
  const raw = (10 - seconds) * 2;
  if (raw <= 0) return 0;
  return raw > IFSS1BT_POINTS_MAX_COLLEGE
    ? IFSS1BT_POINTS_MAX_COLLEGE
    : raw;
}

function computeCollegeInfield(metrics: MetricMap) {
  const rlc2b = computeRlcTotalForPrefixCollege(metrics, "rlc2b");
  const rlc3b = computeRlcTotalForPrefixCollege(metrics, "rlc3b");
  const rlcss = computeRlcTotalForPrefixCollege(metrics, "rlcss");
  ...
  const iff2bPoints = clamp(
    iff2bRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );
  const iff3bPoints = clamp(
    iff3bRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );
  const iffssPoints = clamp(
    iffssRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );

  const ild2bPoints = clamp(
    ild2bRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );
  const ild3bPoints = clamp(
    ild3bRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );
  const ildssPoints = clamp(
    ildssRaw,
    0,
    IF_FLY_POINTS_MAX_COLLEGE
  );
  ...
  const ifss1btPoints = timeTo14_5PointsCollege(ifss1btSeconds);
  ...
  const infieldScore =
    infieldTotalPoints !== null
      ? Number(
          ((infieldTotalPoints / INFIELD_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;

  return {
    score: infieldScore,
    max_points: INFIELD_POINTS_MAX_COLLEGE,
...
/* OUTFIELD */
const C30X30M_POINTS_MAX_COLLEGE = 20;
const T120FT_POINTS_MAX_COLLEGE = 20;
const OFGBHT_POINTS_MAX_COLLEGE = 15;
const OUTFIELD_POINTS_MAX_COLLEGE =
  C30X30M_POINTS_MAX_COLLEGE +
  T120FT_POINTS_MAX_COLLEGE +
  OFGBHT_POINTS_MAX_COLLEGE; // 55

function computeCollegeOutfield(metrics: MetricMap) {
  ...
  const c30x30mPoints = clamp(
    c30x30mRaw,
    0,
    C30X30M_POINTS_MAX_COLLEGE
  );
  const t120Points = clamp(
    t120Raw,
    0,
    T120FT_POINTS_MAX_COLLEGE
  );
  ...
  if (ofgbhtSeconds !== null) {
    ...
    if (raw <= 0) ofgbhtPoints = 0;
      ...
      ofgbhtPoints =
        raw > OFGBHT_POINTS_MAX_COLLEGE ? OFGBHT_POINTS_MAX_COLLEGE : raw;
  }

  const outfieldTotalPoints = sum([
    c30x30mPoints,
    t120Points,
    ofgbhtPoints,
  ]);
  ...
  const outfieldScore =
    outfieldTotalPoints !== null
      ? Number(
          ((outfieldTotalPoints / OUTFIELD_POINTS_MAX_COLLEGE) * 50).toFixed(1)
        )
      : null;

  return {
    score: outfieldScore,
    max_points: OUTFIELD_POINTS_MAX_COLLEGE,
...
/* -------------------------------------------------------------------------- */
/*                        MAIN COLLEGE RATING ENTRYPOINT                      */
/* -------------------------------------------------------------------------- */

export function computeCollegeRatings(metrics: MetricMap): RatingResult {
  const athletic = computeCollegeAthleticSkills(metrics);
  const hitting = computeCollegeHitting(metrics);
  const pitching = computeCollegePitching(metrics);
  const catcher = computeCollegeCatcher(metrics);
  const firstbase = computeCollegeFirstBase(metrics);
  const infield = computeCollegeInfield(metrics);
  const outfield = computeCollegeOutfield(metrics);
