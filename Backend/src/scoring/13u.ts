 ...
 *
 * Same as 12U except:
 * - Vert jump max points: 23 (was 22 at 12U)
 */

const TS1B_MAX_POINTS_13U = 55;
const TS4B_MAX_POINTS_13U = 60;
const SPEED_POINTS_MAX_13U = TS1B_MAX_POINTS_13U + TS4B_MAX_POINTS_13U; // 115

const PUSHUPS_MAX_POINTS_13U = 20; // APUSH60 / 3, max 60 reps
const SITUPS_MAX_POINTS_13U = 15;  // ASIT60 / 4, max 60 reps
const VJUMP_MAX_POINTS_13U = 23;   // updated from 22 → 23

const SLS_OPEN_MAX_13U = 10;
const SLS_CLOSED_MAX_13U = 15;

const MSR_TOTAL_MAX_13U = 6;
const TOE_TOUCH_MAX_13U = 6;
const DEEP_SQUAT_MAX_13U = 9;

const ATHLETIC_POINTS_MAX_13U =
  SPEED_POINTS_MAX_13U +
  PUSHUPS_MAX_POINTS_13U +
  SITUPS_MAX_POINTS_13U +
  VJUMP_MAX_POINTS_13U +
  SLS_OPEN_MAX_13U +
  SLS_CLOSED_MAX_13U +
  MSR_TOTAL_MAX_13U +
  TOE_TOUCH_MAX_13U +
  DEEP_SQUAT_MAX_13U; // 219

function compute13UAthleticSkills(metrics: MetricMap) {
  ...
  const run1bPoints = clamp(run1bPointsRaw, 0, TS1B_MAX_POINTS_13U);
  const run4bPoints = clamp(run4bPointsRaw, 0, TS4B_MAX_POINTS_13U);

  const speedPointsTotal = sum([run1bPoints, run4bPoints]);
  const speedScore =
    speedPointsTotal !== null
      ? Number(((speedPointsTotal / SPEED_POINTS_MAX_13U) * 50).toFixed(1))
      : null;
  ...
  const slsOpenPoints = clamp(slsOpenPointsRaw, 0, SLS_OPEN_MAX_13U);
  const slsClosedPoints = clamp(slsClosedPointsRaw, 0, SLS_CLOSED_MAX_13U);
  ...
  const pushupsPoints = clamp(pushupsPointsRaw, 0, PUSHUPS_MAX_POINTS_13U);
  const situpsPoints = clamp(situpsPointsRaw, 0, SITUPS_MAX_POINTS_13U);

  // Vertical jump
  const vjumpInches = getMetric(metrics, "asp_jump_inches");
  const vjumpPoints = clamp(vjumpInches, 0, VJUMP_MAX_POINTS_13U);
  ...
  const msrPointsClamped =
    msrPointsTotal !== null
      ? Math.max(0, Math.min(MSR_TOTAL_MAX_13U, msrPointsTotal))
      : null;
  ...
  const toeTouchPoints = clamp(toeTouchRawPoints, 0, TOE_TOUCH_MAX_13U);
  const deepSquatPoints = clamp(deepSquatRawPoints, 0, DEEP_SQUAT_MAX_13U);
  ...
  const athleticScore =
    athleticTotalPoints !== null
      ? Number(((athleticTotalPoints / ATHLETIC_POINTS_MAX_13U) * 50).toFixed(1))
      : null;

  return {
    score: athleticScore,
    max_points: ATHLETIC_POINTS_MAX_13U,
...
const TEE_LD_MAX_POINTS_13U = 10;   // HC10LD
const PITCH_QUALITY_MAX_POINTS_13U = 50;  // H10FAST
const EXIT_VELO_MAX_POINTS_13U = 15;      // HPTEV
const BAT_SPEED_MAX_POINTS_13U = 18;      // HPBS
const VARIED_SPEED_MAX_POINTS_13U = 25;   // H5VSPD

const CONTACT_POINTS_MAX_13U = 85; // 10 + 50 + 25
const POWER_POINTS_MAX_13U = 83;   // 15 + 50 + 18
const HITTING_POINTS_MAX_13U = 118; // from eval sheet

function compute13UHitting(metrics: MetricMap) {
  ...
  const pitchPoints = clamp(fastballRaw, 0, PITCH_QUALITY_MAX_POINTS_13U);
  const teeLdPoints = clamp(teeLdRaw, 0, TEE_LD_MAX_POINTS_13U);
  const variedSpeedPoints = clamp(
    variedSpeedRaw,
    0,
    VARIED_SPEED_MAX_POINTS_13U
  );
  ...
  const exitVeloPoints = clamp(
    exitVeloPointsRaw,
    0,
    EXIT_VELO_MAX_POINTS_13U
  );
  const batSpeedPoints = clamp(
    batSpeedPointsRaw,
    0,
    BAT_SPEED_MAX_POINTS_13U
  );
  ...
  const contactScore =
    contactRawPoints !== null
      ? Number(((contactRawPoints / CONTACT_POINTS_MAX_13U) * 100).toFixed(1))
      : null;

  const powerScore =
    powerRawPoints !== null
      ? Number(((powerRawPoints / POWER_POINTS_MAX_13U) * 100).toFixed(1))
      : null;
  ...
  const hittingScore =
    hittingTotalPoints !== null
      ? Number(
          ((hittingTotalPoints / HITTING_POINTS_MAX_13U) * 50).toFixed(1)
        )
      : null;

  return {
    score: hittingScore,
    max_points: HITTING_POINTS_MAX_13U,
...
const PITCH_TSPEED_POINTS_MAX_13U = 37.5; // 75 / 2
const PITCH_ACC_POINTS_MAX_13U = 60;      // TPITCH2060
const PITCH_POINTS_MAX_13U =
  PITCH_TSPEED_POINTS_MAX_13U + PITCH_ACC_POINTS_MAX_13U; // 97.5

const STRIKE_BASELINE_13U = -2.9;
const STRIKE_WEIGHT_SPEED_13U = 1.05;
const STRIKE_WEIGHT_ACC_13U = 0.9;
const STRIKE_MIN_SPEED_13U = 47.5;
const STRIKE_MAX_SPEED_13U = 75;

function compute13UPitching(metrics: MetricMap) {
  ...
  const tspeedPoints = clamp(tspeedPointsRaw, 0, PITCH_TSPEED_POINTS_MAX_13U);

  const tpitchPoints = clamp(tpitchRaw, 0, PITCH_ACC_POINTS_MAX_13U);

  const pitchingTotalPoints = sum([tspeedPoints, tpitchPoints]);

  const pitchingScore =
    pitchingTotalPoints !== null
      ? Number(((pitchingTotalPoints / PITCH_POINTS_MAX_13U) * 50).toFixed(1))
      : null;

  const pitchSpeedScorePercent =
    tspeedPoints !== null
      ? Number(
          ((tspeedPoints / PITCH_TSPEED_POINTS_MAX_13U) * 100).toFixed(1)
        )
      : null;
  ...
  const pitchAccScorePercent =
    tpitchPoints !== null
      ? Number(
          ((tpitchPoints / PITCH_ACC_POINTS_MAX_13U) * 100).toFixed(1)
        )
      : null;
  ...
  const pitchScorePercent =
    tspeedPoints !== null && tpitchPoints !== null
      ? Number(
          (
            ((tspeedPoints + tpitchPoints) / PITCH_POINTS_MAX_13U) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  if (tspeedMph !== null && tpitchPoints !== null) {
    const nSRaw =
      (tspeedMph - STRIKE_MIN_SPEED_13U) /
      (STRIKE_MAX_SPEED_13U - STRIKE_MIN_SPEED_13U);
    const nS = Math.max(0, Math.min(1, nSRaw));
    const dA = tpitchPoints / PITCH_ACC_POINTS_MAX_13U;

    const x =
      STRIKE_BASELINE_13U +
      STRIKE_WEIGHT_SPEED_13U * nS +
      STRIKE_WEIGHT_ACC_13U * dA;
  ...
  return {
    score: pitchingScore,
    max_points: PITCH_POINTS_MAX_13U,
 ...
 * 13U CATCHER
 *
 * Same structure and maxima as 12U:
 * - C10PCS max: 20
 * - CT2BT max: 15
 * - CTTT2B max: 15 (5 throws, 3 pts each)
 */

const C10PCS_POINTS_MAX_13U = 20;
const CT2BT_POINTS_MAX_13U = 15;
const CTTT2B_POINTS_MAX_13U = 15;
const CATCHER_POINTS_MAX_13U =
  C10PCS_POINTS_MAX_13U + CT2BT_POINTS_MAX_13U + CTTT2B_POINTS_MAX_13U; // 50

function compute13UCatcher(metrics: MetricMap) {
  ...
  const c10pcsPoints = clamp(c10pcsRaw, 0, C10PCS_POINTS_MAX_13U);

  let ct2btPoints: number | null = null;
  if (ct2btSeconds !== null) {
    const raw = (5 - ct2btSeconds) * 4; // same structure as 12U
    ct2btPoints = raw <= 0 ? 0 : raw;
    if (ct2btPoints > CT2BT_POINTS_MAX_13U) {
      ct2btPoints = CT2BT_POINTS_MAX_13U;
    }
  }

  const cttt2bPoints = clamp(cttt2bRaw, 0, CTTT2B_POINTS_MAX_13U);

  const catcherTotalPoints = sum([c10pcsPoints, ct2btPoints, cttt2bPoints]);

  const catcherScore =
    catcherTotalPoints !== null
      ? Number(((catcherTotalPoints / CATCHER_POINTS_MAX_13U) * 50).toFixed(1))
      : null;

  return {
    score: catcherScore,
    max_points: CATCHER_POINTS_MAX_13U,
...
const C101B_POINTS_MAX_13U = 30;
const C1BST_POINTS_MAX_13U = 15;
const RLC_POINTS_MAX_13U = 12; // shared with infield RLCs
const FBFLY_POINTS_MAX_13U = 6;
const FBLD_POINTS_MAX_13U = 6;
const FIRSTBASE_POINTS_MAX_13U =
  C101B_POINTS_MAX_13U +
  C1BST_POINTS_MAX_13U +
  RLC_POINTS_MAX_13U +
  FBFLY_POINTS_MAX_13U +
  FBLD_POINTS_MAX_13U; // 69

function computeRlcTotalForPrefix13U(metrics: MetricMap, prefix: string) {
  ...
  const total =
    totalRaw !== null
      ? Math.max(0, Math.min(RLC_POINTS_MAX_13U, totalRaw))
      : null;

  return { pointsByRep: points, total };
}

function compute13UFirstBase(metrics: MetricMap) {
  const c101bRaw = getMetric(metrics, "c101b_catching_test");
  const c1bstRaw = getMetric(metrics, "c1bst_scoops_test");

  const c101bPoints = clamp(c101bRaw, 0, C101B_POINTS_MAX_13U);
  const c1bstPoints = clamp(c1bstRaw, 0, C1BST_POINTS_MAX_13U);

  const rlc1b = computeRlcTotalForPrefix13U(metrics, "rlc1b");

  const fbFlyRaw = getMetric(metrics, "fbfly_points");
  const fbLdRaw = getMetric(metrics, "fbld_points");

  const fbFlyPoints = clamp(fbFlyRaw, 0, FBFLY_POINTS_MAX_13U);
  const fbLdPoints = clamp(fbLdRaw, 0, FBLD_POINTS_MAX_13U);
  ...
  const firstBaseScore =
    firstBaseTotalPoints !== null
      ? Number(
          ((firstBaseTotalPoints / FIRSTBASE_POINTS_MAX_13U) * 50).toFixed(1)
        )
      : null;

  return {
    score: firstBaseScore,
    max_points: FIRSTBASE_POINTS_MAX_13U,
...
const IF_FLY_POINTS_MAX_13U = 6; // each IF fly / LD
const IFSS1BT_POINTS_MAX_13U = 14.5;
const INFIELD_POINTS_MAX_13U = 86.5;

function timeTo14_5Points13U(seconds: number | null): number | null {
  if (seconds === null) return null;
  const raw = (10 - seconds) * 2;
  if (raw <= 0) return 0;
  return raw > IFSS1BT_POINTS_MAX_13U ? IFSS1BT_POINTS_MAX_13U : raw;
}

function compute13UInfield(metrics: MetricMap) {
  const rlc2b = computeRlcTotalForPrefix13U(metrics, "rlc2b");
  const rlc3b = computeRlcTotalForPrefix13U(metrics, "rlc3b");
  const rlcss = computeRlcTotalForPrefix13U(metrics, "rlcss");
  ...
  const iff2bPoints = clamp(iff2bRaw, 0, IF_FLY_POINTS_MAX_13U);
  const iff3bPoints = clamp(iff3bRaw, 0, IF_FLY_POINTS_MAX_13U);
  const iffssPoints = clamp(iffssRaw, 0, IF_FLY_POINTS_MAX_13U);

  const ild2bPoints = clamp(ild2bRaw, 0, IF_FLY_POINTS_MAX_13U);
  const ild3bPoints = clamp(ild3bRaw, 0, IF_FLY_POINTS_MAX_13U);
  const ildssPoints = clamp(ildssRaw, 0, IF_FLY_POINTS_MAX_13U);

  const ifss1btSeconds = getMetric(metrics, "ifss1bt_seconds");
  const ifss1btPoints = timeTo14_5Points13U(ifss1btSeconds);
  ...
  const infieldScore =
    infieldTotalPoints !== null
      ? Number(((infieldTotalPoints / INFIELD_POINTS_MAX_13U) * 50).toFixed(1))
      : null;

  return {
    score: infieldScore,
    max_points: INFIELD_POINTS_MAX_13U,
...
const C20X20M_POINTS_MAX_13U = 20;
const T100FT_POINTS_MAX_13U = 20;
const OFGBHT_POINTS_MAX_13U = 14.5;
const OUTFIELD_POINTS_MAX_13U = 54.5;

function compute13UOutfield(metrics: MetricMap) {
  ...
  const c20x20mPoints = clamp(c20x20mRaw, 0, C20X20M_POINTS_MAX_13U);
  const t100Points = clamp(t100Raw, 0, T100FT_POINTS_MAX_13U);
  ...
  if (ofgbhtSeconds !== null) {
    ...
    if (raw <= 0) ofgbhtPoints = 0;
      ...
      raw > OFGBHT_POINTS_MAX_13U ? OFGBHT_POINTS_MAX_13U : raw;
  }
  ...
  const outfieldScore =
    outfieldTotalPoints !== null
      ? Number(
          ((outfieldTotalPoints / OUTFIELD_POINTS_MAX_13U) * 50).toFixed(1)
        )
      : null;

  return {
    score: outfieldScore,
    max_points: OUTFIELD_POINTS_MAX_13U,
...
/* -------------------------------------------------------------------------- */
/*                          MAIN 13U RATING ENTRYPOINT                        */
/* -------------------------------------------------------------------------- */

export function compute13URatings(metrics: MetricMap): RatingResult {
  const athletic = compute13UAthleticSkills(metrics);
  const hitting = compute13UHitting(metrics);
  const pitching = compute13UPitching(metrics);
  const catcher = compute13UCatcher(metrics);
  const firstbase = compute13UFirstBase(metrics);
  const infield = compute13UInfield(metrics);
  const outfield = compute13UOutfield(metrics);
