 ...
 * PRO ATHLETIC
 *
 * Same as HS/College except:
 * - 1B speed max: 70 pts
 * - 4B speed max: 75 pts
 * - ASPJUMP max: 35 inches
 * - ASPSCP max: 35 ft
 * - ASPSUP max: 35 ft
 *
 * Strength, balance, mobility same as HS/College.
 */

const TS1B_MAX_POINTS_PRO = 70;
const TS4B_MAX_POINTS_PRO = 75;
const SPEED_POINTS_MAX_PRO =
  TS1B_MAX_POINTS_PRO + TS4B_MAX_POINTS_PRO; // 145

const PUSHUPS_MAX_POINTS_PRO = 25; // APUSH60 / 3, max 75 reps
const SITUPS_MAX_POINTS_PRO = 25; // ASIT60 / 4, max 100 reps
const PULLUPS_MAX_POINTS_PRO = 25; // APULL60, 1 pt per pull-up up to 25

const VJUMP_MAX_POINTS_PRO = 35; // ASPJUMP, inches

const ASPSCP_MAX_POINTS_PRO = 35; // seated chest pass, ft
const ASPSUP_MAX_POINTS_PRO = 35; // sit-up and throw, ft

const SLS_OPEN_MAX_PRO = 10;
const SLS_CLOSED_MAX_PRO = 15;

const MSR_TOTAL_MAX_PRO = 6;
const TOE_TOUCH_MAX_PRO = 6;
const DEEP_SQUAT_MAX_PRO = 9;

// Category-level maxima
const STRENGTH_POINTS_MAX_PRO =
  PUSHUPS_MAX_POINTS_PRO +
  SITUPS_MAX_POINTS_PRO +
  PULLUPS_MAX_POINTS_PRO; // 75

const APOWER_POINTS_MAX_PRO =
  VJUMP_MAX_POINTS_PRO +
  ASPSCP_MAX_POINTS_PRO +
  ASPSUP_MAX_POINTS_PRO; // 105

const BALANCE_POINTS_MAX_PRO =
  SLS_OPEN_MAX_PRO + SLS_CLOSED_MAX_PRO; // 25

const MOBILITY_POINTS_MAX_PRO =
  MSR_TOTAL_MAX_PRO + TOE_TOUCH_MAX_PRO + DEEP_SQUAT_MAX_PRO; // 21

const ATHLETIC_POINTS_MAX_PRO =
  SPEED_POINTS_MAX_PRO +
  PUSHUPS_MAX_POINTS_PRO +
  SITUPS_MAX_POINTS_PRO +
  PULLUPS_MAX_POINTS_PRO +
  VJUMP_MAX_POINTS_PRO +
  ASPSCP_MAX_POINTS_PRO +
  ASPSUP_MAX_POINTS_PRO +
  SLS_OPEN_MAX_PRO +
  SLS_CLOSED_MAX_PRO +
  MSR_TOTAL_MAX_PRO +
  TOE_TOUCH_MAX_PRO +
  DEEP_SQUAT_MAX_PRO; // 371

function computeProAthleticSkills(metrics: MetricMap) {
  ...
  const run1bPoints = clamp(run1bPointsRaw, 0, TS1B_MAX_POINTS_PRO);
  const run4bPoints = clamp(run4bPointsRaw, 0, TS4B_MAX_POINTS_PRO);

  const speedPointsTotal = sum([run1bPoints, run4bPoints]);
  const speedScore =
    speedPointsTotal !== null
      ? Number(((speedPointsTotal / SPEED_POINTS_MAX_PRO) * 50).toFixed(1))
      : null;
  ...
  const slsOpenPoints = clamp(slsOpenPointsRaw, 0, SLS_OPEN_MAX_PRO);
  const slsClosedPoints = clamp(slsClosedPointsRaw, 0, SLS_CLOSED_MAX_PRO);

  const balancePointsTotal = sum([slsOpenPoints, slsClosedPoints]);
  const balanceScore =
    balancePointsTotal !== null
      ? Number(((balancePointsTotal / BALANCE_POINTS_MAX_PRO) * 50).toFixed(1))
      : null;
  ...
  const pushupsPoints = clamp(pushupsPointsRaw, 0, PUSHUPS_MAX_POINTS_PRO);
  const situpsPoints = clamp(situpsPointsRaw, 0, SITUPS_MAX_POINTS_PRO);
  const pullupsPoints = clamp(pullupsPointsRaw, 0, PULLUPS_MAX_POINTS_PRO);
  ...
  const strengthScore =
    strengthPointsTotal !== null
      ? Number(
          ((strengthPointsTotal / STRENGTH_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  // Vertical jump
  const vjumpInches = getMetric(metrics, "asp_jump_inches");
  const vjumpPoints = clamp(vjumpInches, 0, VJUMP_MAX_POINTS_PRO);

  // Power tests
  const aspscpDistanceFt = getMetric(metrics, "aspscp_distance_ft");
  const aspscpMedBallWeight = getMetric(metrics, "aspscp_med_ball_weight");
  const aspscpPoints = clamp(aspscpDistanceFt, 0, ASPSCP_MAX_POINTS_PRO);

  const aspsupDistanceFt = getMetric(metrics, "aspsup_distance_ft");
  const aspsupMedBallWeight = getMetric(metrics, "aspsup_med_ball_weight");
  const aspsupPoints = clamp(aspsupDistanceFt, 0, ASPSUP_MAX_POINTS_PRO);

  const apowerPointsTotal = sum([vjumpPoints, aspscpPoints, aspsupPoints]);
  const apowerScore =
    apowerPointsTotal !== null
      ? Number(((apowerPointsTotal / APOWER_POINTS_MAX_PRO) * 50).toFixed(1))
      : null;
  ...
  const msrPointsClamped =
    msrPointsTotal !== null
      ? Math.max(0, Math.min(MSR_TOTAL_MAX_PRO, msrPointsTotal))
      : null;
  ...
  const toeTouchPoints = clamp(toeTouchRawPoints, 0, TOE_TOUCH_MAX_PRO);
  const deepSquatPoints = clamp(deepSquatRawPoints, 0, DEEP_SQUAT_MAX_PRO);
  ...
  const mobilityScore =
    mobilityPointsTotal !== null
      ? Number(
          ((mobilityPointsTotal / MOBILITY_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;
  ...
  const athleticScore =
    athleticTotalPoints !== null
      ? Number(
          ((athleticTotalPoints / ATHLETIC_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: athleticScore,
    max_points: ATHLETIC_POINTS_MAX_PRO,
    ...
    breakdown: {
      tests: {
        ...
        speed_points_max: SPEED_POINTS_MAX_PRO,
        ...
        strength_points_max: STRENGTH_POINTS_MAX_PRO,
        ...
        power_points_max: APOWER_POINTS_MAX_PRO,
        ...
        balance_points_max: BALANCE_POINTS_MAX_PRO,
        ...
        mobility_points_max: MOBILITY_POINTS_MAX_PRO,
...
/* -------------------------------------------------------------------------- */
/**
 * All hitting tests & maxima same as HS/College.
 */

const TEE_LD_MAX_POINTS_PRO = 10;        // HC10LD
const PITCH_QUALITY_MAX_POINTS_PRO = 50; // H10FAST
const EXIT_VELO_MAX_POINTS_PRO = 24;     // HPTEV
const BAT_SPEED_MAX_POINTS_PRO = 25;     // HBSPEED
const VARIED_SPEED_MAX_POINTS_PRO = 25;  // H5VSPD
const CURVEBALL_MAX_POINTS_PRO = 25;     // H5CBPD

const CONTACT_POINTS_MAX_PRO =
  TEE_LD_MAX_POINTS_PRO +
  PITCH_QUALITY_MAX_POINTS_PRO +
  VARIED_SPEED_MAX_POINTS_PRO +
  CURVEBALL_MAX_POINTS_PRO; // 110

const POWER_POINTS_MAX_PRO =
  EXIT_VELO_MAX_POINTS_PRO +
  PITCH_QUALITY_MAX_POINTS_PRO +
  BAT_SPEED_MAX_POINTS_PRO; // 99

const HITTING_POINTS_MAX_PRO =
  TEE_LD_MAX_POINTS_PRO +
  PITCH_QUALITY_MAX_POINTS_PRO +
  EXIT_VELO_MAX_POINTS_PRO +
  BAT_SPEED_MAX_POINTS_PRO +
  VARIED_SPEED_MAX_POINTS_PRO +
  CURVEBALL_MAX_POINTS_PRO; // 159

function computeProHitting(metrics: MetricMap) {
  ...
  const curveballRaw = getMetric(metrics, "m_5_curveball_quality");

  const pitchPoints = clamp(fastballRaw, 0, PITCH_QUALITY_MAX_POINTS_PRO);
  const teeLdPoints = clamp(teeLdRaw, 0, TEE_LD_MAX_POINTS_PRO);
  const variedSpeedPoints = clamp(
    variedSpeedRaw,
    0,
    VARIED_SPEED_MAX_POINTS_PRO
  );
  const curveballPoints = clamp(curveballRaw, 0, CURVEBALL_MAX_POINTS_PRO);

  const exitVeloPointsRaw = exitVeloMph !== null ? exitVeloMph / 5 : null;
  const batSpeedPointsRaw = batSpeedMph !== null ? batSpeedMph / 4 : null;

  const exitVeloPoints = clamp(exitVeloPointsRaw, 0, EXIT_VELO_MAX_POINTS_PRO);
  const batSpeedPoints = clamp(batSpeedPointsRaw, 0, BAT_SPEED_MAX_POINTS_PRO);
  ...
  const contactScore =
    contactRawPoints !== null
      ? Number(
          ((contactRawPoints / CONTACT_POINTS_MAX_PRO) * 100).toFixed(1)
        )
      : null;

  const powerScore =
    powerRawPoints !== null
      ? Number(((powerRawPoints / POWER_POINTS_MAX_PRO) * 100).toFixed(1))
      : null;
  ...
  const hittingScore =
    hittingTotalPoints !== null
      ? Number(
          ((hittingTotalPoints / HITTING_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: hittingScore,
    max_points: HITTING_POINTS_MAX_PRO,
 ...
 * PRO PITCHING
 *
 * Same as HS/College but:
 * - TSPEED max points: 52.5 (105 mph / 2)
 * - Logistic params for STRIKEOUTCHANCE (Pro):
 *   baseline = -4.2
 *   a_s = 1.6
 *   a_a = 1.4
 *   a_p = 0.35
 ...
 *   maxSpeed = 105 mph
 *
 * Additional pitches (TPITCH5AP1, 2, 3, ...) are supported by
 * scanning keys starting with "tpitch5ap".
 */

const PITCH_TSPEED_POINTS_MAX_PRO = 52.5; // 105 / 2
const PITCH_ACC_POINTS_MAX_PRO = 60; // TPITCH2060
const PITCH_POINTS_MAX_PRO =
  PITCH_TSPEED_POINTS_MAX_PRO + PITCH_ACC_POINTS_MAX_PRO; // 112.5

const ADDITIONAL_PITCH_POINTS_MAX_PRO = 15;

const STRIKE_BASELINE_PRO = -4.2;
const STRIKE_WEIGHT_SPEED_PRO = 1.6;
const STRIKE_WEIGHT_ACC_PRO = 1.4;
const STRIKE_WEIGHT_P_PRO = 0.35;
const STRIKE_WEIGHT_AP_ACC_PRO = 0.9;
const STRIKE_MIN_SPEED_PRO = 75;
const STRIKE_MAX_SPEED_PRO = 105;

function computeProPitching(metrics: MetricMap) {
  ...
  const tspeedPoints = clamp(
    tspeedPointsRaw,
    0,
    PITCH_TSPEED_POINTS_MAX_PRO
  );

  const tpitchPoints = clamp(tpitchRaw, 0, PITCH_ACC_POINTS_MAX_PRO);

  // Additional pitch tests: tpitch5ap1, tpitch5ap2, ...
  ...
  for (const key of Object.keys(metrics as any)) {
    ...
    const clamped = clamp(raw, 0, ADDITIONAL_PITCH_POINTS_MAX_PRO);
  ...
  const additionalPitchAccuracyNormalized =
    additionalPitchAveragePoints !== null
      ? Math.max(
          ...
          Math.min(
            1,
            additionalPitchAveragePoints / ADDITIONAL_PITCH_POINTS_MAX_PRO
          )
        )
      : 0;
  ...
  const pitchingScore =
    pitchingTotalPoints !== null
      ? Number(
          ((pitchingTotalPoints / PITCH_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;
  ...
  const pitchSpeedScorePercent =
    tspeedPoints !== null
      ? Number(
          (
            (tspeedPoints / PITCH_TSPEED_POINTS_MAX_PRO) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  const pitchAccScorePercent =
    tpitchPoints !== null
      ? Number(
          (
            (tpitchPoints / PITCH_ACC_POINTS_MAX_PRO) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  const pitchScorePercent =
    tspeedPoints !== null && tpitchPoints !== null
      ? Number(
          (
            ((tspeedPoints + tpitchPoints) / PITCH_POINTS_MAX_PRO) *
            100
          ).toFixed(1)
        )
      : null;
  ...
  if (tspeedMph !== null && tpitchPoints !== null) {
    const nSRaw =
      (tspeedMph - STRIKE_MIN_SPEED_PRO) /
      (STRIKE_MAX_SPEED_PRO - STRIKE_MIN_SPEED_PRO);
    const nS = Math.max(0, Math.min(1, nSRaw));

    const accNorm =
      tpitchPoints !== null
        ? Math.max(0, Math.min(1, tpitchPoints / PITCH_ACC_POINTS_MAX_PRO))
        : 0;
    ...
    const x =
      STRIKE_BASELINE_PRO +
      STRIKE_WEIGHT_SPEED_PRO * nS +
      STRIKE_WEIGHT_ACC_PRO * accNorm +
      STRIKE_WEIGHT_P_PRO * P +
      STRIKE_WEIGHT_AP_ACC_PRO * AA;
  ...
  return {
    score: pitchingScore,
    max_points: PITCH_POINTS_MAX_PRO,
...
/* -------------------------------------------------------------------------- */
/*                      PRO CATCHER / 1B / IF / OF                            */
/* -------------------------------------------------------------------------- */
/**
 * These are identical to HS/College in both maxima and calculations.
 */

/* CATCHER */
const C20PCS_POINTS_MAX_PRO = 40;
const CT2BT_POINTS_MAX_PRO = 15;
const CTTT2B_POINTS_MAX_PRO = 15;
const CATCHER_POINTS_MAX_PRO =
  C20PCS_POINTS_MAX_PRO +
  CT2BT_POINTS_MAX_PRO +
  CTTT2B_POINTS_MAX_PRO; // 70

function computeProCatcher(metrics: MetricMap) {
  ...
  const c20pcsPoints = clamp(c20pcsRaw, 0, C20PCS_POINTS_MAX_PRO);

  let ct2btPoints: number | null = null;
  if (ct2btSeconds !== null) {
    const raw = (5 - ct2btSeconds) * 4;
    ct2btPoints = raw <= 0 ? 0 : raw;
    if (ct2btPoints > CT2BT_POINTS_MAX_PRO) {
      ct2btPoints = CT2BT_POINTS_MAX_PRO;
    }
  }

  const cttt2bPoints = clamp(cttt2bRaw, 0, CTTT2B_POINTS_MAX_PRO);
  ...
  const catcherScore =
    catcherTotalPoints !== null
      ? Number(
          ((catcherTotalPoints / CATCHER_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: catcherScore,
    max_points: CATCHER_POINTS_MAX_PRO,
...
/* FIRST BASE */
const C101B_POINTS_MAX_PRO = 30;
const C1BST_POINTS_MAX_PRO = 15;
const RLC_POINTS_MAX_PRO = 12;
const FBFLY_POINTS_MAX_PRO = 6;
const FBLD_POINTS_MAX_PRO = 6;
const FIRSTBASE_POINTS_MAX_PRO =
  C101B_POINTS_MAX_PRO +
  C1BST_POINTS_MAX_PRO +
  RLC_POINTS_MAX_PRO +
  FBFLY_POINTS_MAX_PRO +
  FBLD_POINTS_MAX_PRO; // 69

function computeRlcTotalForPrefixPro(metrics: MetricMap, prefix: string) {
  ...
  const total =
    totalRaw !== null
      ? Math.max(0, Math.min(RLC_POINTS_MAX_PRO, totalRaw))
      : null;

  return { pointsByRep: points, total };
}

function computeProFirstBase(metrics: MetricMap) {
  const c101bRaw = getMetric(metrics, "c101b_catching_test");
  const c1bstRaw = getMetric(metrics, "c1bst_scoops_test");

  const c101bPoints = clamp(c101bRaw, 0, C101B_POINTS_MAX_PRO);
  const c1bstPoints = clamp(c1bstRaw, 0, C1BST_POINTS_MAX_PRO);

  const rlc1b = computeRlcTotalForPrefixPro(metrics, "rlc1b");

  const fbFlyRaw = getMetric(metrics, "fbfly_points");
  const fbLdRaw = getMetric(metrics, "fbld_points");

  const fbFlyPoints = clamp(fbFlyRaw, 0, FBFLY_POINTS_MAX_PRO);
  const fbLdPoints = clamp(fbLdRaw, 0, FBLD_POINTS_MAX_PRO);
  ...
  const firstBaseScore =
    firstBaseTotalPoints !== null
      ? Number(
          ((firstBaseTotalPoints / FIRSTBASE_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: firstBaseScore,
    max_points: FIRSTBASE_POINTS_MAX_PRO,
...
/* INFIELD */
const IF_FLY_POINTS_MAX_PRO = 6;
const IFSS1BT_POINTS_MAX_PRO = 14.5;
const INFIELD_POINTS_MAX_PRO = 86.5;

function timeTo14_5PointsPro(seconds: number | null): number | null {
  if (seconds === null) return null;
  const raw = (10 - seconds) * 2;
  if (raw <= 0) return 0;
  return raw > IFSS1BT_POINTS_MAX_PRO ? IFSS1BT_POINTS_MAX_PRO : raw;
}

function computeProInfield(metrics: MetricMap) {
  const rlc2b = computeRlcTotalForPrefixPro(metrics, "rlc2b");
  const rlc3b = computeRlcTotalForPrefixPro(metrics, "rlc3b");
  const rlcss = computeRlcTotalForPrefixPro(metrics, "rlcss");
  ...
  const iff2bPoints = clamp(iff2bRaw, 0, IF_FLY_POINTS_MAX_PRO);
  const iff3bPoints = clamp(iff3bRaw, 0, IF_FLY_POINTS_MAX_PRO);
  const iffssPoints = clamp(iffssRaw, 0, IF_FLY_POINTS_MAX_PRO);

  const ild2bPoints = clamp(ild2bRaw, 0, IF_FLY_POINTS_MAX_PRO);
  const ild3bPoints = clamp(ild3bRaw, 0, IF_FLY_POINTS_MAX_PRO);
  const ildssPoints = clamp(ildssRaw, 0, IF_FLY_POINTS_MAX_PRO);

  const ifss1btSeconds = getMetric(metrics, "ifss1bt_seconds");
  const ifss1btPoints = timeTo14_5PointsPro(ifss1btSeconds);
  ...
  const infieldScore =
    infieldTotalPoints !== null
      ? Number(
          ((infieldTotalPoints / INFIELD_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: infieldScore,
    max_points: INFIELD_POINTS_MAX_PRO,
...
/* OUTFIELD */
const C30X30M_POINTS_MAX_PRO = 20;
const T120FT_POINTS_MAX_PRO = 20;
const OFGBHT_POINTS_MAX_PRO = 15;
const OUTFIELD_POINTS_MAX_PRO =
  C30X30M_POINTS_MAX_PRO +
  T120FT_POINTS_MAX_PRO +
  OFGBHT_POINTS_MAX_PRO; // 55

function computeProOutfield(metrics: MetricMap) {
  ...
  const c30x30mPoints = clamp(c30x30mRaw, 0, C30X30M_POINTS_MAX_PRO);
  const t120Points = clamp(t120Raw, 0, T120FT_POINTS_MAX_PRO);
  ...
  if (ofgbhtSeconds !== null) {
    ...
    if (raw <= 0) ofgbhtPoints = 0;
      ...
      ofgbhtPoints =
        raw > OFGBHT_POINTS_MAX_PRO ? OFGBHT_POINTS_MAX_PRO : raw;
  }
  ...
  const outfieldScore =
    outfieldTotalPoints !== null
      ? Number(
          ((outfieldTotalPoints / OUTFIELD_POINTS_MAX_PRO) * 50).toFixed(1)
        )
      : null;

  return {
    score: outfieldScore,
    max_points: OUTFIELD_POINTS_MAX_PRO,
...
/* -------------------------------------------------------------------------- */
/*                        MAIN PRO RATING ENTRYPOINT                          */
/* -------------------------------------------------------------------------- */

export function computeProRatings(metrics: MetricMap): RatingResult {
  const athletic = computeProAthleticSkills(metrics);
  const hitting = computeProHitting(metrics);
  const pitching = computeProPitching(metrics);
  const catcher = computeProCatcher(metrics);
  const firstbase = computeProFirstBase(metrics);
  const infield = computeProInfield(metrics);
  const outfield = computeProOutfield(metrics);
