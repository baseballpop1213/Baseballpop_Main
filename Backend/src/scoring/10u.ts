...
/* -------------------------------------------------------------------------- */
/**
 * Same structure as 9U, but updated maxima:
 * - Speed points max: 45 + 45 = 90
 * - Push-ups: 20
 * - Sit-ups: 15
 * - Vert jump: 20
 * - SLS open: 10
 ...
 * - Toe touch: 6
 * - Deep squat: 9
 * - Category max total: 191
 */

const TS1B_MAX_POINTS_10U = 45;
const TS4B_MAX_POINTS_10U = 45;
const SPEED_POINTS_MAX_10U = TS1B_MAX_POINTS_10U + TS4B_MAX_POINTS_10U; // 90

const PUSHUPS_MAX_POINTS_10U = 20;
const SITUPS_MAX_POINTS_10U = 15;
const VJUMP_MAX_POINTS_10U = 20;

const SLS_OPEN_MAX_10U = 10;
const SLS_CLOSED_MAX_10U = 15;

const MSR_TOTAL_MAX_10U = 6;
const TOE_TOUCH_MAX_10U = 6;
const DEEP_SQUAT_MAX_10U = 9;

const ATHLETIC_POINTS_MAX_10U =
  SPEED_POINTS_MAX_10U +
  PUSHUPS_MAX_POINTS_10U +
  SITUPS_MAX_POINTS_10U +
  VJUMP_MAX_POINTS_10U +
  SLS_OPEN_MAX_10U +
  SLS_CLOSED_MAX_10U +
  MSR_TOTAL_MAX_10U +
  TOE_TOUCH_MAX_10U +
  DEEP_SQUAT_MAX_10U; // 191

function compute10UAthleticSkills(metrics: MetricMap) {
  // Speed: 1B & 4B, now using time + distance (with defaults 60ft / 240ft)
  ...
  const run1bPoints = clamp(run1bPointsRaw, 0, TS1B_MAX_POINTS_10U);
  const run4bPoints = clamp(run4bPointsRaw, 0, TS4B_MAX_POINTS_10U);

  const speedPointsTotal = sum([run1bPoints, run4bPoints]);
  const speedScore =
    speedPointsTotal !== null
      ? Number(((speedPointsTotal / SPEED_POINTS_MAX_10U) * 50).toFixed(1))
      : null;
  ...
  const slsOpenPoints = clamp(slsOpenPointsRaw, 0, SLS_OPEN_MAX_10U);
  const slsClosedPoints = clamp(slsClosedPointsRaw, 0, SLS_CLOSED_MAX_10U);
  ...
  const pushupsPoints = clamp(pushupsPointsRaw, 0, PUSHUPS_MAX_POINTS_10U);
  const situpsPoints = clamp(situpsPointsRaw, 0, SITUPS_MAX_POINTS_10U);

  // Vertical jump
  const vjumpInches = getMetric(metrics, "asp_jump_inches");
  const vjumpPoints = clamp(vjumpInches, 0, VJUMP_MAX_POINTS_10U);
  ...
  const msrPointsClamped =
    msrPointsTotal !== null
      ? Math.max(0, Math.min(MSR_TOTAL_MAX_10U, msrPointsTotal))
      : null;
  ...
  const toeTouchPoints = clamp(toeTouchRawPoints, 0, TOE_TOUCH_MAX_10U);
  const deepSquatPoints = clamp(deepSquatRawPoints, 0, DEEP_SQUAT_MAX_10U);
  ...
  const athleticScore =
    athleticTotalPoints !== null
      ? Number(((athleticTotalPoints / ATHLETIC_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  return {
    score: athleticScore,
    max_points: ATHLETIC_POINTS_MAX_10U,
...
/* -------------------------------------------------------------------------- */
/**
 * Same structure as 9U with updated HPBS / HPTEV maxima.
 */

const TEE_LD_MAX_POINTS_10U = 10; // HC10LD
const PITCH_QUALITY_MAX_POINTS_10U = 50; // H10FAST
const EXIT_VELO_MAX_POINTS_10U = 15; // HPTEV
const BAT_SPEED_MAX_POINTS_10U = 18; // HPBS

const CONTACT_POINTS_MAX_10U = 60; // tee LD + fastball quality
const POWER_POINTS_MAX_10U = 83; // EV + pitch + bat speed
const HITTING_POINTS_MAX_10U = 93; // full category (tee LD + EV + pitch + bat speed)

function compute10UHitting(metrics: MetricMap) {
  ...
  const pitchPoints = clamp(fastballRaw, 0, PITCH_QUALITY_MAX_POINTS_10U);
  const teeLdPoints = clamp(teeLdRaw, 0, TEE_LD_MAX_POINTS_10U);

  const exitVeloPointsRaw = exitVeloMph !== null ? exitVeloMph / 5 : null;
  const batSpeedPointsRaw = batSpeedMph !== null ? batSpeedMph / 4 : null;

  const exitVeloPoints = clamp(exitVeloPointsRaw, 0, EXIT_VELO_MAX_POINTS_10U);
  const batSpeedPoints = clamp(batSpeedPointsRaw, 0, BAT_SPEED_MAX_POINTS_10U);
  ...
  const contactScore =
    contactRawPoints !== null
      ? Number(((contactRawPoints / CONTACT_POINTS_MAX_10U) * 100).toFixed(1))
      : null;

  const powerScore =
    powerRawPoints !== null
      ? Number(((powerRawPoints / POWER_POINTS_MAX_10U) * 100).toFixed(1))
      : null;
  ...
  const hittingScore =
    hittingTotalPoints !== null
      ? Number(((hittingTotalPoints / HITTING_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  return {
    score: hittingScore,
    max_points: HITTING_POINTS_MAX_10U,
 ...
 * New logistic parameters provided:
 *   baseline = -2.5
 *   a_s = 0.9
 *   a_a = 0.7
 *   minSpeed = 40 mph
...
const PITCH_TSPEED_POINTS_MAX_10U = 32.5; // 65 / 2
const PITCH_ACC_POINTS_MAX_10U = 30;
const PITCH_POINTS_MAX_10U =
  PITCH_TSPEED_POINTS_MAX_10U + PITCH_ACC_POINTS_MAX_10U; // 62.5

const STRIKE_BASELINE_10U = -2.5;
const STRIKE_WEIGHT_SPEED_10U = 0.9;
const STRIKE_WEIGHT_ACC_10U = 0.7;
const STRIKE_MIN_SPEED_10U = 40;
const STRIKE_MAX_SPEED_10U = 65;

function compute10UPitching(metrics: MetricMap) {
  ...
  const tspeedPoints = clamp(tspeedPointsRaw, 0, PITCH_TSPEED_POINTS_MAX_10U);

  const tpitchPoints = clamp(tpitchRaw, 0, PITCH_ACC_POINTS_MAX_10U);

  const pitchingTotalPoints = sum([tspeedPoints, tpitchPoints]);

  const pitchingScore =
    pitchingTotalPoints !== null
      ? Number(((pitchingTotalPoints / PITCH_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  const pitchSpeedScorePercent =
    tspeedPoints !== null
      ? Number(
          ((tspeedPoints / PITCH_TSPEED_POINTS_MAX_10U) * 100).toFixed(1)
        )
      : null;
  ...
  const pitchAccScorePercent =
    tpitchPoints !== null
      ? Number(
          ((tpitchPoints / PITCH_ACC_POINTS_MAX_10U) * 100).toFixed(1)
        )
      : null;
  ...
  const pitchScorePercent =
    tspeedPoints !== null && tpitchPoints !== null
      ? Number(
          (((tspeedPoints + tpitchPoints) / PITCH_POINTS_MAX_10U) * 100).toFixed(
            1
          )
        )
      : null;
  ...
  if (tspeedMph !== null && tpitchPoints !== null) {
    const nSRaw =
      (tspeedMph - STRIKE_MIN_SPEED_10U) /
      (STRIKE_MAX_SPEED_10U - STRIKE_MIN_SPEED_10U);
    const nS = Math.max(0, Math.min(1, nSRaw));
    const dA = tpitchPoints / PITCH_ACC_POINTS_MAX_10U;

    const x =
      STRIKE_BASELINE_10U +
      STRIKE_WEIGHT_SPEED_10U * nS +
      STRIKE_WEIGHT_ACC_10U * dA;
  ...
  return {
    score: pitchingScore,
    max_points: PITCH_POINTS_MAX_10U,
...
/* -------------------------------------------------------------------------- */
/**
 * 10U CATCHER (CT2BT + C10PCS)
 */

const C10PCS_POINTS_MAX_10U = 20;
const CT2BT_POINTS_MAX_10U = 15;
const CATCHER_POINTS_MAX_10U =
  C10PCS_POINTS_MAX_10U + CT2BT_POINTS_MAX_10U; // 35

function compute10UCatcher(metrics: MetricMap) {
  const c10pcsRaw = getMetric(metrics, "c10pcs_points");
  const ct2btSeconds = getMetric(metrics, "ct2bt_seconds");

  const c10pcsPoints = clamp(c10pcsRaw, 0, C10PCS_POINTS_MAX_10U);

  let ct2btPoints: number | null = null;
  if (ct2btSeconds !== null) {
    const raw = (5 - ct2btSeconds) * 4; // same structure as 9U CB2BT
    ct2btPoints = raw <= 0 ? 0 : raw;
    if (ct2btPoints > CT2BT_POINTS_MAX_10U) {
      ct2btPoints = CT2BT_POINTS_MAX_10U;
    }
  }
  ...
  const catcherScore =
    catcherTotalPoints !== null
      ? Number(((catcherTotalPoints / CATCHER_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  return {
    score: catcherScore,
    max_points: CATCHER_POINTS_MAX_10U,
...
/* -------------------------------------------------------------------------- */
/**
 * C101B (0–30) + C1BST (0–15) → max 45 points
 */

const C101B_POINTS_MAX_10U = 30;
const C1BST_POINTS_MAX_10U = 15;
const FIRSTBASE_POINTS_MAX_10U =
  C101B_POINTS_MAX_10U + C1BST_POINTS_MAX_10U; // 45

function compute10UFirstBase(metrics: MetricMap) {
  const c101bRaw = getMetric(metrics, "c101b_catching_test");
  const c1bstRaw = getMetric(metrics, "c1bst_scoops_test");

  const c101bPoints = clamp(c101bRaw, 0, C101B_POINTS_MAX_10U);
  const c1bstPoints = clamp(c1bstRaw, 0, C1BST_POINTS_MAX_10U);

  const firstBaseTotalPoints = sum([c101bPoints, c1bstPoints]);

  const firstBaseScore =
    firstBaseTotalPoints !== null
      ? Number(((firstBaseTotalPoints / FIRSTBASE_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  return {
    score: firstBaseScore,
    max_points: FIRSTBASE_POINTS_MAX_10U,
...
const RLC_POINTS_MAX_10U = 12; // per position
const IF_FLY_POINTS_MAX_10U = 6; // each IF fly / LD
const IFSS1BT_POINTS_MAX_10U = 14.5;
const INFIELD_POINTS_MAX_10U = 86.5;

function computeRlcTotalForPrefix10U(metrics: MetricMap, prefix: string) {
  ...
  const total =
    totalRaw !== null
      ? Math.max(0, Math.min(RLC_POINTS_MAX_10U, totalRaw))
      : null;

  return { pointsByRep: points, total };
}

function timeTo14_5Points(seconds: number | null): number | null {
  if (seconds === null) return null;
  const raw = (10 - seconds) * 2;
  if (raw <= 0) return 0;
  return raw > 14.5 ? 14.5 : raw;
}

function compute10UInfield(metrics: MetricMap) {
  const rlc2b = computeRlcTotalForPrefix10U(metrics, "rlc2b");
  const rlc3b = computeRlcTotalForPrefix10U(metrics, "rlc3b");
  const rlcss = computeRlcTotalForPrefix10U(metrics, "rlcss");
  ...
  const iff2bPoints = clamp(iff2bRaw, 0, IF_FLY_POINTS_MAX_10U);
  const iff3bPoints = clamp(iff3bRaw, 0, IF_FLY_POINTS_MAX_10U);
  const iffssPoints = clamp(iffssRaw, 0, IF_FLY_POINTS_MAX_10U);

  const ild2bPoints = clamp(ild2bRaw, 0, IF_FLY_POINTS_MAX_10U);
  const ild3bPoints = clamp(ild3bRaw, 0, IF_FLY_POINTS_MAX_10U);
  const ildssPoints = clamp(ildssRaw, 0, IF_FLY_POINTS_MAX_10U);

  const ifss1btSeconds = getMetric(metrics, "ifss1bt_seconds");
  const ifss1btPoints = timeTo14_5Points(ifss1btSeconds);
  ...
  const infieldScore =
    infieldTotalPoints !== null
      ? Number(((infieldTotalPoints / INFIELD_POINTS_MAX_10U) * 50).toFixed(1))
      : null;

  return {
    score: infieldScore,
    max_points: INFIELD_POINTS_MAX_10U,
...
const C20X20M_POINTS_MAX_10U = 20;
const T80FT_POINTS_MAX_10U = 20;
const OFGBHT_POINTS_MAX_10U = 14.5;
const OUTFIELD_POINTS_MAX_10U = 54.5;

function compute10UOutfield(metrics: MetricMap) {
  ...
  const c20x20mPoints = clamp(c20x20mRaw, 0, C20X20M_POINTS_MAX_10U);
  const t80Points = clamp(t80Raw, 0, T80FT_POINTS_MAX_10U);

  let ofgbhtPoints: number | null = null;
  if (ofgbhtSeconds !== null) {
    const raw = (10 - ofgbhtSeconds) * 2;
    if (raw <= 0) ofgbhtPoints = 0;
    else ofgbhtPoints = raw > OFGBHT_POINTS_MAX_10U ? OFGBHT_POINTS_MAX_10U : raw;
  }
  ...
  const outfieldScore =
    outfieldTotalPoints !== null
      ? Number(
          ((outfieldTotalPoints / OUTFIELD_POINTS_MAX_10U) * 50).toFixed(1)
        )
      : null;

  return {
    score: outfieldScore,
    max_points: OUTFIELD_POINTS_MAX_10U,
...
/* -------------------------------------------------------------------------- */
/*                          MAIN 10U RATING ENTRYPOINT                        */
/* -------------------------------------------------------------------------- */

export function compute10URatings(metrics: MetricMap): RatingResult {
  const athletic = compute10UAthleticSkills(metrics);
  const hitting = compute10UHitting(metrics);
  const pitching = compute10UPitching(metrics);
  const catcher = compute10UCatcher(metrics);
  const firstbase = compute10UFirstBase(metrics);
  const infield = compute10UInfield(metrics);
  const outfield = compute10UOutfield(metrics);
